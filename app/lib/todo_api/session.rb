class TodoApi::Session < TodoApi::Application
  ...
  attr_reader :user_id, :auth_token, :response
  ...
  def get_user(user_login_params)
    ...
    @user_id = @response_object[:user_id] if @response_object[:user_id]
    @auth_token = @response_object[:auth_token] if @response_object[:auth_token]
  end
